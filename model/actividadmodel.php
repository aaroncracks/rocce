...
class actividad_model{
    ...
    public function get_actividades1(){
        $id = $_GET["id"];
        $consulta=$this->db->query("SELECT actividades.id, actividades.nombre, actividades.descripcion, habilitado, lugares.nombre as nom_lugar, actividades.imagen From actividades left join lugares on actividades.lugar_id=lugares.id 
